...
int main(int argc, char *argv[]) {
  ...
  options.add_options()
    ...
    (
      ...
      "Set packet type flag - \"telemetry\"|\"telecommand\"|<int (0-"
        + std::to_string((int)pow(2, TYPE_FLAG_LEN)-1)
        + ")>",
    ...
    (
      ...
      "Set secondary header flag. 1 indicates the presence of a secondary header, 0 its absence - <int (0-"
        + std::to_string((int)pow(2, SEC_HDR_FLAG_LEN)-1)
        + ")>",
      cxxopts::value<int>()->default_value("0")
    )
    (
      ...
      "Set application process id - \"aqua_modis\"|<int (0-"
        + std::to_string((int)pow(2, APP_ID_LEN)-1)
        + ")>",
      cxxopts::value<std::string>()->default_value("0")
    )
    (
      ...
      "Set sequence flags, incidating whether the packets' data is a segment of a larger set - \"first\"|\"last\"|\"continuation\"|\"unsegmented\"|<int (0-"
        + std::to_string((int)pow(2, SEQ_FLAGS_LEN)-1)
        + ")>",
      cxxopts::value<std::string>()->default_value("0")
    )
    (
      ...
      "Set packet sequence count or packet name - <int (0-"
        + std::to_string((int)pow(2, SEQ_CNT_OR_NAME_LEN)-1)
        + ")>",
      cxxopts::value<int>()->default_value("0")
    )
    ("h,help", "Print usage")
  ...
  if (result["type-flag"].as<std::string>() == "telemetry") {
    ...
    try {
      ...
      if (type >= pow(2, TYPE_FLAG_LEN)) {
        std::cerr << "Error: type must be between 0 and " << pow(2, TYPE_FLAG_LEN)-1 << std::endl;
        valid = false;
      }
    }
    catch(std::invalid_argument) {
      std::cerr << "Error: type-flag must be either \"telemetry\", \"telecommand\", or <int (0-" << pow(2, TYPE_FLAG_LEN)-1 << ")>" << std::endl;
      valid = false;
    }
  ...
  if (result["sec-hdr-flag"].as<int>() >= pow(2, SEC_HDR_FLAG_LEN)) {
    std::cerr << "Error: sec-hdr-flag must be between 0 and " << pow(2, SEC_HDR_FLAG_LEN)-1 << std::endl;
    valid = false;
  }
  ...
  try {
    ...
    try {
      ...
      if (app_id >= pow(2, APP_ID_LEN)) {
        std::cerr << "Error: app-id must be between 0 and " << pow(2, APP_ID_LEN)-1 << std::endl;
        valid = false;
      }
    }
    catch(std::invalid_argument) {
  ...
  if (result["seq-flags"].as<std::string>() == "first") {
    ...
    try {
      ...
      if (seq_flags >= pow(2, TYPE_FLAG_LEN)) {
        std::cerr << "Error: seq-flags must be between 0 and " << pow(2, TYPE_FLAG_LEN)-1 << std::endl;
        valid = false;
      }
    }
    catch(std::invalid_argument) {
      std::cerr << "Error: seq-flags must be either \"telemetry\", \"telecommand\", or <int (0-" << pow(2, TYPE_FLAG_LEN)-1 << ")>" << std::endl;
      valid = false;
    }
    catch(std::out_of_range) {
  ...
  if (result["seq-cnt-or-name"].as<int>() >= pow(2, SEQ_CNT_OR_NAME_LEN)) {
    std::cerr << "Error: seq-cnt-or-name must be between 0 and " << pow(2, SEQ_CNT_OR_NAME_LEN)-1 << std::endl;
    valid = false;
  }
