...
Accordingly, a growing market for satellite-derived datasets has emerged which process data for specific purposes including forest fire monitoring, dust storm detection, and flood tracking~\cite{sarikhani2021new}.
...
\textbf{TODO maybe rewrite first sentence.}
We apply some of these concepts to the novel target of packet-based satellite downlinks; we demonstrate that signal spoofing is possible for satellites for which the receiving and broadcasting equipment was previously thought to make such attacks prohibitively expensive, and show that attacks can have a significant effect on processing systems downstream from the original data.
